...
gcloud config set project billingindiaclients
gcloud services enable cloudresourcemanager.googleapis.com
gcloud auth application-default set-quota-project billingindiaclients
printf "\nINFO - GCP project set to 'billingindiaclients' succesfully!\n"

BUCKET_EXISTS=$(gcloud storage ls gs://billingindiaclients-vigenair > /dev/null 2>&1 && echo "true" || echo "false")
if "${BUCKET_EXISTS}"; then
  printf "\nWARN - Bucket 'billingindiaclients-vigenair' already exists. Skipping bucket creation...\n"
else
  gcloud storage buckets create gs://billingindiaclients-vigenair --project=billingindiaclients --location=us --uniform-bucket-level-access
  test $? -eq 0 || exit
  printf "\nINFO - Bucket 'billingindiaclients-vigenair' created successfully in location 'us'!\n"
fi
...
PROJECT_NUMBER=$(gcloud projects describe billingindiaclients --format="value(projectNumber)")
...
for SA in "aiplatform.googleapis.com" "storage.googleapis.com" "eventarc.googleapis.com"; do
    gcloud --no-user-output-enabled beta services identity create --project=billingindiaclients \
        --service="${SA}"
done
COMPUTE_SA_ROLES=(
...
--trigger-event-filters="bucket=billingindiaclients-vigenair" \
